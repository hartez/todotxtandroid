        ...
        if (expanded) {
            LazyColumn(
                ...
                items(options) { option ->
                    FilterOption(text = option, null, selectedOption == option) { onSelected(option) }
                }
            }
        }
    }
...
// TODO This is looking promising, but we need to fix the spacing when there's not icon
...
fun ExpandingOptionPreview() {
    TodotxtAndroidTheme {
        Surface {
            ExpandingOption(
                "Test", Icons.AutoMirrored.Outlined.Label,
